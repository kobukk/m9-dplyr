...
num.of.rows <- nrow(team.data)
num.of.col <- ncol(team.data)

col <- colnames(team.data)
col
...
sorted <- arrange(team.data, Ratio)
lowest.ratio <- filter(team.data, Ratio == min(Ratio))
lowest.ratio
...
team.data <- mutate(team.data, Assists.Per.Game = AST / G) %>% arrange(-Assists.Per.Game)
...
offensive.stats <- select(team.data, ORB, FG, AST)
...
BetterShooters <- function (team.1, team.2){
  ...
  best.team <- filter(team.data, Team %in% c(team.1, team.2))%>%
    filter(FG. == max(FG.)) %>%
    select(Team, FG., PTS)
